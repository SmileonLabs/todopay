...
h3('1.1 플랫폼 소개');
para('TodoPay는 가상계좌 기반 구매 처리 및 매장 정산 관리를 위한 다단계 핀테크 어드민 플랫폼입니다. 슈퍼어드민부터 매장까지 5단계 계층 구조로 운영되며, 회원이 가상계좌로 구매금액을 입금하면 수수료를 차감 후 해당 매장 잔액에 자동 적립됩니다. 매장은 적립된 잔액을 출금 신청하여 정산받습니다.');
y += 4;
card3([
...
h3('5.1 수수료 종류');
para('TodoPay의 수수료는 세 가지 유형으로 구성됩니다. 각 유형은 독립적으로 설정되며, 구매 확인 시 자동 계산됩니다.');
table(
  ['수수료 종류', '단위', '설명', '적용 시점'],
  [
    ['입금수수료', '원 (정액)', '구매 1건당 고정 차감액. 회원→매장 구간에서 차감', '구매 확인 처리 시'],
    ['이용수수료율', '% (비율)', '구매금액 × 요율. 계층별 마진 차등 배분', '구매 확인 처리 시'],
    ['출금수수료', '원 (정액)', '매장 출금 1건당 고정 차감액', '매장 출금 신청 시'],
  ],
  [110, 75, 185, 105]
);
infoBox('ℹ️ 매장에 수수료가 설정되지 않은 경우 입금수수료 0원, 이용수수료율 0%가 적용됩니다. 운영 전 반드시 설정하세요.');

h3('5.2 이용수수료율 계층 배분 구조');
para('이용수수료율은 상위 계층이 하위 계층에 배정합니다. 각 계층은 상위에서 배정받은 요율 이상으로만 설정할 수 있으며, 구매 확인 시 각 계층의 마진(상위요율 − 내요율)에 해당하는 금액이 자동 배분됩니다.');

const nodes = [
  { label: '슈퍼어드민', val: '상한없음', color: '#7c3aed', tc: WHITE },
  { label: '본사 배정', val: '3.0%', color: NAVY2, tc: BLUE },
  { label: '총판 설정', val: '2.0%', color: '#0369a1', tc: WHITE },
  { label: '대리점 설정', val: '1.5%', color: '#0891b2', tc: WHITE },
  { label: '매장 적용', val: '1.0%', color: '#0f766e', tc: WHITE },
];
...
h4('마진 배분 예시 (구매금액 100,000원, 입금수수료 500원)');
table(
  ['계층', '적용 요율', '마진율', '배분 금액'],
  [
    ['매장', '1.0%', '—', '입금수수료 500원 차감 후 98,000원 매장 잔액 적립'],
    ['대리점', '1.5%', '0.5%', '100,000 × 0.5% = 500원 수익'],
    ['총판', '2.0%', '0.5%', '100,000 × 0.5% = 500원 수익'],
    ['본사', '3.0%', '1.0%', '100,000 × 1.0% = 1,000원 수익'],
  ],
  [80, 80, 80, 255]
);
infoBox('ℹ️ 계산식: 매장 적립액 = 구매금액 − 입금수수료 − (구매금액 × 매장 이용수수료율)\n예) 100,000 − 500 − (100,000 × 1.0%) = 98,500원');

h3('5.3 수수료 설정 방법');
step(1, '수수료 설정 메뉴 이동', '좌측 메뉴에서 "수수료 설정"을 클릭합니다.');
step(2, '대상 역할 탭 선택', '상단 탭(본사/총판/대리점/매장)에서 수수료를 설정할 역할을 선택합니다.');
step(3, '수수료 입력', '입금수수료(원), 이용수수료율(%), 출금수수료(원)를 각 항목에 입력합니다. 상위 배정 이용수수료율(상한선)이 함께 표시되며, 이보다 낮게 설정 시 오류가 반환됩니다.');
...
h3('7.2 구매 확인 시 수수료 계산');
infoBox('구매금액 100,000원, 입금수수료 500원, 이용수수료율 2% 기준\n\n입금수수료: 500원 (정액 차감)\n이용수수료: 100,000 × 2% = 2,000원\n총 수수료: 2,500원\n매장 적립액: 100,000 − 2,500 = 97,500원');
...
infoBox('ℹ️ 출금 가능 시간: 신청 다음 날 오전 10:00 (KST 기준)\n출금 가능 시간 이전에는 승인 버튼이 비활성화되며, 해당 시간이 표시됩니다.\n\n출금수수료: 출금 신청 금액에서 정액 차감\n예) 출금 신청 50,000원, 출금수수료 500원 → 실지급 49,500원');
