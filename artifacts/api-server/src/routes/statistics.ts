...
router.get("/statistics/overview", async (_req, res) => {
  ...
  const [todayDeposit] = await db.select({ amount: sql<number>`coalesce(sum(amount), 0)` })
    .from(transactionsTable).where(and(eq(transactionsTable.type, "deposit"), gte(transactionsTable.createdAt, today)));
  const [todayWithdrawal] = await db.select({ amount: sql<number>`coalesce(sum(amount), 0)` })
    .from(transactionsTable).where(and(eq(transactionsTable.type, "withdrawal"), gte(transactionsTable.createdAt, today)));
  const [todayFee] = await db.select({ amount: sql<number>`coalesce(sum(fee), 0)` })
    .from(transactionsTable).where(gte(transactionsTable.createdAt, today));
  const [monthDeposit] = await db.select({ amount: sql<number>`coalesce(sum(amount), 0)` })
    .from(transactionsTable).where(and(eq(transactionsTable.type, "deposit"), gte(transactionsTable.createdAt, monthStart)));
  const [monthWithdrawal] = await db.select({ amount: sql<number>`coalesce(sum(amount), 0)` })
    .from(transactionsTable).where(and(eq(transactionsTable.type, "withdrawal"), gte(transactionsTable.createdAt, monthStart)));
...
router.get("/statistics/daily", async (req, res) => {
  ...
  const conditions = [gte(transactionsTable.createdAt, startDate), lte(transactionsTable.createdAt, endDate)];
  const txs = await db.select().from(transactionsTable).where(and(...conditions));

  const byDate = new Map<string, { depositCount: number; depositAmount: number; withdrawalCount: number; withdrawalAmount: number; feeAmount: number }>();
  for (const t of txs) {
    const date = t.createdAt.toISOString().split("T")[0];
    if (!byDate.has(date)) byDate.set(date, { depositCount: 0, depositAmount: 0, withdrawalCount: 0, withdrawalAmount: 0, feeAmount: 0 });
    const d = byDate.get(date)!;
    if (t.type === "deposit") { d.depositCount++; d.depositAmount += Number(t.amount); }
    else { d.withdrawalCount++; d.withdrawalAmount += Number(t.amount); }
    d.feeAmount += Number(t.fee);
  }

  const result = Array.from(byDate.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([date, d]) => ({
    date,
    ...
    netAmount: d.depositAmount - d.withdrawalAmount - d.feeAmount,
  }));
